sub EVENT_ENTERZONE {
  if((plugin::HasClass("Enchanter")) && plugin::check_hasitem($client, 52963)) { #Sullied Gold Filigree
    if(!$entity_list->GetNPCByNPCTypeID(211047) && !defined($qglobals{anthone})) {
      quest::spawn2(211047,0,0,-1853,2479,-110,40); ##Anthone_Chapin
    }
  }

  if((plugin::HasClass("Magician")) && !defined($qglobals{mage_epic_hoh}) && defined($qglobals{mage_epic}) && $client->GetGlobal("mage_epic") ==10) {
...
sub EVENT_LOOT {
  if ((plugin::HasClass("Magician")) && $looted_id == 19547) {  
