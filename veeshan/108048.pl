...
sub EVENT_ITEM {
	if ($faction == 1) {
		if (plugin::check_handin(\%itemcount, 69316 => 1)) { #Box of Phara Dar (Full)
			...
			if (plugin::HasClass("Cleric") || plugin::HasClass("Druid") || plugin::HasClass("Shaman") || plugin::HasClass("Necromancer") || plugin::HasClass("Wizard") || plugin::HasClass("Magician") || plugin::HasClass("Enchanter")) { #Caster
			quest::summonitem(69390); #Earring of Phara Dar
			} elsif (plugin::HasClass("Warrior") || plugin::HasClass("Paladin") || plugin::HasClass("Ranger") || plugin::HasClass("Shadowknight") || plugin::HasClass("Monk") || plugin::HasClass("Bard") || plugin::HasClass("Rogue") || plugin::HasClass("Beastlord") || plugin::HasClass("Berserker")) { #Melee/Hybrid
