...
sub EVENT_SAY {
  if ($text=~/hail/i) {
	if(plugin::HasClass("Ranger") && (plugin::check_hasitem($client, 20488) || plugin::check_hasitem($client, 62600))) {
  ...
  if ($text=~/out of balance/i){
	if (plugin::HasClass("Ranger") && $client->GetGlobal("ranger_epic") ==1) {
	...
	elsif(plugin::HasClass("Druid") && plugin::check_hasitem($client, 20490) or plugin::check_hasitem($client, 62809)) { #has druid 1.0 OR Seed of Wrath
